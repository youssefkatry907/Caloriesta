...
export async function list(query: object = {}) {
  const records = await Coupon.find(query);
  return records;
}
...
export async function create(form: any) {
  if (await isNameDuplicate(form["code"])) {
    ...
    const newRecord = new Coupon(form);
...
export async function update(id, form: any) {
  ...
  if (id && isFound.success) {
    await Coupon.updateOne({ _id: id }, form, {
      runValidators: true,
    });
  }
...
async function isNameDuplicate(code: string) {
  const record = await Coupon.findOne({ code: code });
  return record;
}

async function isExist(id: string) {
  const record = await Coupon.findOne({ _id: id });
  if (record) {
    return {
      success: true,
      record,
