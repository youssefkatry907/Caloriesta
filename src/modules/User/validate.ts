...
export function validateCustomer(payload: any, res: Response) {
  let errors = [];
...
export function validateConsultant(data: any, res: Response) {
...
export function isValidAppUsersTypes(data: string, res: Response) {
  if (
    ![
      "customer",
      "nutrition",
    ...
    res.status(400).json(
      resTypes(400, {
        errors: {
          key: "type",
          message:
            "type not valid it must one of customer, nutrition, fitness, resturant ",
        },
      })
    );
  else return true;
