...
export async function list(query: object = {}) {
  const records = await Industry.find(query);
  return records;
}
...
export async function create(form: any) {
  if (await isNameDuplicate(form["name"])) {
    ...
    const newRecord = new Industry(form);
...
export async function update(id, form: any) {
  ...
  if (id && isFound.success) {
    await Industry.updateOne({ _id: id }, form, {
      runValidators: true,
    });
  }
...
async function isNameDuplicate(name: string) {
  const record = await Industry.findOne({ name: name });
  return record;
}

async function isExist(id: string) {
  const record = await Industry.findOne({ _id: id });
  if (record) {
    return {
      success: true,
      record,
