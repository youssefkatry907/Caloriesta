...
export async function list(query: object = {}) {
  const records = await Filter.find(query);
  return records;
}
...
export async function create(form: any) {
  if (await isNameDuplicate(form["name"])) {
    ...
    const newRecord = new Filter(form);
...
export async function update(id, form: any) {
  ...
  if (id && isFound.success) {
    await Filter.updateOne({ _id: id }, form, {
      runValidators: true,
    });
  }
...
export async function remove(id) {
  const isFound = await isExist(id);
  if (id && isFound.success) {
    await Filter.deleteOne({ _id: id });
  }
  return isFound;
}

async function isNameDuplicate(name: string) {
  const record = await Filter.findOne({ name: name });
  return record;
}

async function isExist(id: string) {
  const record = await Filter.findOne({ _id: id });
  if (record) {
    return {
      success: true,
      record,
